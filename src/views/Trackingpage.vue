<template>
 ...
 <tbody>
      <tr v-for="(Tracking_data_list_data, index) in Tracking_data_list" :key="index">
        ...
        <td>{{Tracking_data_list_data.tracking_id}}</td>
        <td>
...
<td>{{Tracking_data_list_data.shipment_status}}</td>

        <td>{{Tracking_data_list_data.companyprofit}}</td>
...
data(){
    return{
        ...
        tracking_statement:'',
        History_data_list:[],
        Tracking_data_list_data_specific_order_id:'',
    }
},
created(){
  ...
  async tracking_list_collection(){
            onSnapshot(query(collection(db, 'order_details_for_tracking_and_payment')), (snap) =>{snap.forEach((doc)=>{
               // this.check_tracking = doc.data().tracking_id;
                ...
                if(doc.data().tracking_id != "Input Tracking ID"){
                  this.Tracking_data_list.push(doc.data())
                }
        ...
        async history_collection(){
          onSnapshot(query(collection(db, 'HistoryofOrderplaced')), (snap) =>{snap.forEach((doc)=>{
               // this.check_tracking = doc.data().tracking_id;            
                  this.History_data_list.push(doc.data())
                
               console.log('History_data_list fetched')
            })} )
        },

       async iframe_link(Tracking_data_list_data){
          this.iframe_link_data = Tracking_data_list_data.tracking_id
          this.Tracking_data_list_data_specific_order_id=Tracking_data_list_data.specific_order_id
          await onSnapshot(query(collection(db, 'order_details_for_tracking_and_payment'), where('specific_order_id', '==', this.Tracking_data_list_data_specific_order_id)),
      (check_id) => {
...
await setDoc(doc(db,'order_details_for_tracking_and_payment',this.ID_order_delete), 
                {
...
//if delivered is clicked the admin_database final_net_profit should be adjusted just as the deduction is adjusted and the database for each item
    onSnapshot(query(collection(db, 'order_details_for_tracking_and_payment'),where('specific_order_id', '==', this.ID_order_delete)), (snap) => {
      snap.forEach((doc) => {
...
//via specific_order_id is pushed to history for reference purposes then delete the ones in the order_details_for_tracking_and_payment
              ...
              await setDoc(doc(db,'order_details_for_tracking_and_payment', doc.id), 
                {
                 ...
                 await onSnapshot(query(collection(db, 'order_details_for_tracking_and_payment'),where('specific_order_id', '==', this.ID_order_delete)), (snap) => {
      snap.forEach((doc) => {
        //this.net_profit = doc.data().net_profit;        
         addDoc(collection(db, 'HistoryofOrderplaced'), doc.data())
            //address to this
   ...
   await onSnapshot(query(collection(db, 'list_of_order_details_for_tracking_and_payment'), where('specific_order_id', '==', this.specific_order_id)),
      (check) => {
        check.forEach(async(doc) => {
          this.specific_order_id = doc.data().specific_order_id;
          this.ID_delete = doc.id;

          await setDoc(doc(db, 'list_of_order_details_for_tracking_and_payment', doc.id),
            {
              final_net_profit: this.final_net_profit + this.net_profit,
              history_profit: this.history_profit + this.net_profit,
              time_of_payment_assurance: new Date(),
          ...
          await addDoc(collection(db, 'HistoryofOrderitemsplaced'), doc.data());
            // }).then(() => {
...
onSnapshot(query(collection(db, 'order_details_for_tracking_and_payment'),where('specific_order_id', '==', this.Tracking_data_list_data_specific_order_id)),
      (check_id) => {
        check_id.forEach((doc) => {
          this.specific_order_id = doc.data().specific_order_id;
          this.ID_order_delete = doc.id;
